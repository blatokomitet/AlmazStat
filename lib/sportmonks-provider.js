...
function statisticScalar(value) {
  if (value === null || value === undefined) return null;
  if (typeof value === "number" || typeof value === "string") return value;
  if (typeof value !== "object") return null;
  return value.count ?? value.value ?? value.total ?? value.average ?? null;
}

function splitTeamStatistic(value) {
  const source = value && typeof value === "object" ? value : {};
  return {
    home: statisticScalar(source.home),
    away: statisticScalar(source.away),
    total: statisticScalar(source.all) ?? statisticScalar(value),
  };
}

function normalizeTeamSeasonStatistics(team, seasonId) {
  const rows = Array.isArray(team?.statistics) ? team.statistics : [];
  const seasonRow = rows.find((row) => Number(row?.season_id) === Number(seasonId)) || rows[0] || null;
  const details = (Array.isArray(seasonRow?.details) ? seasonRow.details : []).map((detail) => ({
    id: detail?.id ?? null,
    typeId: detail?.type?.id ?? detail?.type_id ?? null,
  ...
  for (const detail of details) {
    for (const key of [detail.developerName, detail.code, detail.name]) {
      if (key) byType.set(String(key).trim().toUpperCase().replace(/[ -]+/g, "_"), detail);
    }
  }
  const pick = (...keys) => {
    for (const key of keys) {
      const detail = byType.get(key);
      if (detail) return splitTeamStatistic(detail.value);
    }
    return { home: null, away: null, total: null };
  };
  const wins = pick("WIN", "WINS", "TEAM_WINS");
  const draws = pick("DRAW", "DRAWS", "TEAM_DRAWS");
  const losses = pick("LOST", "LOSS", "LOSSES", "TEAM_LOST", "TEAM_LOSSES");
  const played = pick("MATCHES", "MATCHES_PLAYED", "TEAM_MATCHES", "OVERALL_MATCHES");
  for (const part of ["home", "away", "total"]) {
    if (played[part] === null && [wins[part], draws[part], losses[part]].every((value) => Number.isFinite(Number(value)))) {
      played[part] = Number(wins[part]) + Number(draws[part]) + Number(losses[part]);
    }
  }
  return {
    id: seasonRow?.id ?? null,
    seasonId: seasonRow?.season_id ?? Number(seasonId),
    team: normalizeSportmonksTeam(team),
    played,
    wins,
    draws,
    losses,
    goalsFor: { total: pick("GOALS", "GOALS_FOR", "SCORED", "TEAM_GOALS") },
    goalsAgainst: { total: pick("GOALS_CONCEDED", "CONCEDED", "TEAM_GOALS_CONCEDED") },
    cleanSheets: pick("CLEAN_SHEET", "CLEAN_SHEETS"),
    failedToScore: pick("FAILED_TO_SCORE", "TEAM_FAILED_TO_SCORE"),
    details,
  };
}
...
function normalizeSportmonksLeagueEntity(league) {
  ...
  const currentSeason =
    league.currentSeason ?? league.currentseason ?? league.current_season ?? null;
...
function oddsMarketKind(row) {
  const value = [
    ...
    .join(" ")
    .toLowerCase();
  if (/(match winner|full.?time result|three.?way|3.?way|1x2)/.test(value)) return "1x2";
  if (/(over.?under|total goals|goals total|totals?)/.test(value)) return "totals";
  return null;
}

function normalizeSportmonksOdd(row) {
  ...
  if (kind === "1x2") {
    ...
    if (/^(home|1)$/.test(normalized)) option = "1";
    else if (/^(draw|x)$/.test(normalized)) option = "X";
    else if (/^(away|2)$/.test(normalized)) option = "2";
  } else if (total !== null && !rawLabel.includes(String(total))) {
    option = `${rawLabel} ${total}`.trim();
  }
  return {
    bookmaker: row?.bookmaker?.name ?? row?.bookmaker_name ?? null,
    bookmakerId: row?.bookmaker_id ?? row?.bookmaker?.id ?? null,
    market: kind === "1x2" ? "1X2" : "Total goals",
    marketKind: kind,
    option: option || null,
    odd: row?.value ?? row?.odd ?? null,
...
function normalizeFixtureXg(row) {
  return {
    id: row?.id ?? null,
    teamId: row?.participant_id ?? null,
    location: row?.location ?? null,
...
export function normalizeSportmonksMatchCentre(fixture) {
  ...
  const injuries = Array.isArray(fixture?.sidelined)
    ...
    : [];
  const rawXg = fixture?.xGFixture ?? fixture?.xgfixture ?? fixture?.xgFixture;
  const expectedGoals = Array.isArray(rawXg)
    ? rawXg.map(normalizeFixtureXg).filter((row) => row.teamId !== null)
    : [];
...
export function createSportmonksProvider({
  ...
  async function teamStatisticsBySeason(seasonId, teamId) {
    const result = await request(`/teams/seasons/${encodeURIComponent(seasonId)}`, {
    ...
    const team = teams.find((row) => Number(row?.id) === Number(teamId)) || null;
    return {
      statistics: team ? normalizeTeamSeasonStatistics(team, seasonId) : null,
      meta: { provider: "sportmonks", pagination: result.pagination, rateLimit: result.rateLimit },
    };
  }
  ...
  async function oddsByFixture(fixtureId) {
    const result = await request(
      `/odds/pre-match/fixtures/${encodeURIComponent(fixtureId)}`,
      { include: "market;bookmaker" },
    );
    const rows = (Array.isArray(result.data) ? result.data : [])
      .map(normalizeSportmonksOdd)
      .filter(Boolean);
    return {
      odds: rows,
      meta: {
